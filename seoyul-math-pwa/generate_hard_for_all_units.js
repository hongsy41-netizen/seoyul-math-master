// 모든 16개 단원에 난이도 3 문제 대폭 추가 (단원당 최소 10개씩)
const fs = require('fs');

// 16개 단원 정의
const units = [
  { id: 'm1-1-1-I-1', name: '소인수분해', target: 5 }, // 이미 16개 있음, 5개만 추가
  { id: 'm1-1-1-I-2', name: '최대공약수와 최소공배수', target: 5 }, // 이미 16개 있음, 5개만 추가
  { id: 'm1-1-1-II-1', name: '정수와 유리수', target: 7 }, // 이미 13개 있음, 7개 추가
  { id: 'm1-1-1-II-2', name: '정수와 유리수의 계산', target: 7 }, // 이미 13개 있음, 7개 추가
  { id: 'm1-1-1-III-1', name: '문자의 사용과 식', target: 7 }, // 이미 13개 있음, 7개 추가
  { id: 'm1-1-1-III-2', name: '일차식의 계산', target: 7 }, // 이미 13개 있음, 7개 추가
  { id: 'm1-1-1-III-3', name: '방정식과 그 해', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-1-III-4', name: '일차방정식의 풀이', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-1-III-5', name: '일차방정식의 활용', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-I-1', name: '기본 도형', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-I-2', name: '위치 관계', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-I-3', name: '작도와 합동', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-II-1', name: '다각형', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-II-2', name: '원과 부채꼴', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-III-1', name: '다면체와 회전체', target: 19 }, // 1개만 있음, 19개 추가로 총 20개
  { id: 'm1-1-2-III-2', name: '입체도형의 겉넓이와 부피', target: 19 } // 1개만 있음, 19개 추가로 총 20개
];

// 단원별 최상위 문제 생성 함수
const hardProblems = {
  // 1-1학기: 수와 연산, 문자와 식
  'm1-1-1-I-1': [ // 소인수분해 (5개 추가)
    {
  ...
  'm1-1-2-III-2': [ // 입체도형의 겉넓이와 부피 (19개 추가)
    {
...
let output = `// 모든 단원에 최상위 난이도 3 문제 추가 (단원당 최소 10개 이상)
// 총 ${units.reduce((sum, u) => sum + u.target, 0)}개의 새로운 최상위 문제 추가
...
for (const unit of units) {
  const problems = hardProblems[unit.id] || [];
  console.log(`${unit.name} (${unit.id}): ${problems.length}개 문제 추가`);
  
  for (let i = 0; i < problems.length; i++) {
    const p = problems[i];
    output += `window.Templates_hard.push({
  unitId:'${unit.id}',
  difficulty:3,
  type:'word_problem_hard_${unit.id.replace(/[^a-zA-Z0-9]/g, '_')}_${i+1}',
  class:'최상위_${unit.name}',
  gen:function(){
    return {
      question:'${p.question}',
      answer:${typeof p.answer === 'string' ? `'${p.answer}'` : p.answer},
      equation:'${p.equation}',
      hint:'${p.hint}'
    };
  }
...
// 파일 저장
fs.writeFileSync('./js/generator/templates_hard_all_units.js', output, 'utf-8');
console.log(`\n✅ templates_hard_all_units.js 생성 완료!`);
console.log(`총 ${units.reduce((sum, u) => sum + u.target, 0)}개의 최상위 문제 추가됨`);
console.log('\n단원별 추가 현황:');
for (const unit of units) {
  const problems = hardProblems[unit.id] || [];
  console.log(`  ${unit.name}: ${problems.length}개 추가`);
}
